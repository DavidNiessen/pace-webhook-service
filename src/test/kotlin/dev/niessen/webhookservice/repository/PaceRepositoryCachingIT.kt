...
class PaceRepositoryCachingIT : IntegrationTestBase() {
    ...
    fun setup() {
        `when`(
            ...
            ResponseEntity("12345", HttpStatus.OK)
        )
    ...
    @Test
    fun `caches the response and invalidates cache after ttl`() {
        ...
        await().atMost(10, TimeUnit.SECONDS).untilAsserted {
            repository.fetchPaceJson()
            verify(httpClient, times(2)).exchange(
                any<URI>(), any<HttpMethod>(), any<HttpEntity<*>>(), any<Class<*>>()
            )
        }
    }
}
